...
import { setData, setTopChart } from './store/playlistData';
...
const Home = () => {
    ...
    const [fetchingSongs, setFetchingSongs] = useState(null)
    const [errorSongs, setErrorSongs] = useState(null)
    const { user } = useAuthContext()
    const dispatch = useDispatch();
    const songs = useSelector((state) => state.displayPlaylistData.data)
    const topChart = useSelector((state) => state.displayPlaylistData.topChart)
    ...
    const fetchSongs = async () => {
        ...
        try {
            const responses = await Promise.all([
                fetch('https://musica-by-iyin-api.onrender.com/api/song/genre/sort', {
                    headers: {
                        'Authorization': `Bearer ${user.token}`
                    },
                }),
                fetch('https://musica-by-iyin-api.onrender.com/api/playlists', {
                    headers: {
                        'Authorization': `Bearer ${user.token}`
                    },
                })
            ...
            if (!responses[0].ok || !responses[1].ok) {
                setErrorSongs(json[0].error)
                setFetchingSongs(false)
                dispatch(setData(null))
                dispatch(setTopChart(null))
            }

            if (responses[0].ok || responses[1].ok) {
                dispatch(setData(json[0]))
                dispatch(setTopChart(json[1]))
                setFetchingSongs(false)
                setErrorSongs(false)
            }
        } catch (error) {
                    ...
                    <div className="home-section1">
                        ...
                        <div className="topCharts">
                            ...
                            <div className="topchart-playlists">
                                {topChart.slice(0, 3).map((playlist, index) => (
                                    <div className="topChart" key={index}>
                                        <img src={playlist.image} alt="" />
                                        <div className="topChartInfo">
                                            <Link to={`/home/playlist/${playlist._id}`} className="topChartName">{playlist.name}</Link>
                                            <p className="topChartOwn">{playlist.name}</p>
                                            <p className="topChartDur">{convert(playlist.duration / 1000)}</p>
                                        </div>
                    ...
                    {songs.map((song, index) => (
                        <div className="latest" key={index}>
                            <div className='latest-header'>
                                <h2>{song.genre}</h2>
                                <Link to={`/home/genre/${index}`} className="topChartName">See more</Link>
                            </div>
                            <div className="topLatest">
                                {song.songs.slice(0, 10).map((track, index) => (
                                    <div className="newTrack" key={index}>
                                        <img src={track.image.src} alt="" />
                                        <p>{track.title}</p>
                                    </div>
                                ))}
                            </div>
